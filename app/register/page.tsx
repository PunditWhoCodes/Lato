...
export default function RegisterPage() {
      ...
      default:
        return (
          <div className="bg-white rounded-3xl shadow-2xl p-8 md:p-10">
            ...
            {/* Title */}
            <h2 className="text-center text-2xl md:text-3xl font-normal text-black mb-6">
              Create Account
            </h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Name"
                  className="w-full px-6 py-3 border border-[#D9D9D9] rounded-full text-black placeholder:text-[#D9D9D9] focus:outline-none focus:border-[#00A699] transition-colors"
                  required
              ...
              {/* Country Dropdown */}
              <div>
                <label className="block text-sm text-black mb-2">Select Country</label>
                <div className="relative">
                  <button
                    type="button"
                    onClick={() => setShowCountryDropdown(!showCountryDropdown)}
                    disabled={showLoading}
                    className="w-full px-6 py-3 border border-[#D9D9D9] rounded-full text-black text-left flex items-center justify-between focus:outline-none focus:border-[#00A699] transition-colors disabled:opacity-50"
                  >
                    <span>{country}</span>
                    <ChevronDown className={`w-5 h-5 text-black transition-transform ${showCountryDropdown ? 'rotate-180' : ''}`} />
                  </button>
                    ...
                    <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-[#D9D9D9] rounded-xl shadow-lg z-20 max-h-48 overflow-y-auto">
                      {countries.map((c) => (
                        <button
                          ...
                          className="w-full px-4 py-2.5 text-left text-sm text-black hover:bg-[#F9FAFB] transition-colors"
                        >
                          {c.name}
                        </button>
                      ))}
              ...
              {/* Mobile Number Field */}
              <div>
                <label className="block text-sm text-black mb-2">Mobile Number</label>
                <input
                  type="tel"
                  value={mobileNumber}
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full px-6 py-3 border border-[#D9D9D9] rounded-full text-black placeholder:text-[#D9D9D9] focus:outline-none focus:border-[#00A699] transition-colors pr-12"
                    required
                    minLength={6}
                    disabled={showLoading}
                  />
              ...
              <div className="flex items-start gap-2">
                <input
                  ...
                  className="w-4 h-4 mt-0.5 rounded border-black border-2 text-[#00A699] focus:ring-[#00A699]"
                  disabled={showLoading}
                />
                <label htmlFor="terms" className="text-sm text-black">
                  By proceeding, you accept our{" "}
                  <Link href="/terms" className="text-[#00A699] font-medium hover:underline">Terms of Use</Link>
                  {" "}and{" "}
                  <Link href="/privacy" className="text-[#00A699] font-medium hover:underline">Data Policy</Link>
                </label>
              </div>
            ...
            <div className="space-y-3">
              <button
                ...
                disabled={showLoading}
                className="w-full flex items-center justify-center gap-3 px-5 py-3 h-[50px] border border-[#D9D9D9] rounded-full hover:bg-[#F9FAFB] transition-colors disabled:opacity-50"
              >
                ...
                <span className="text-base font-medium text-black">Sign in with Google</span>
              </button>
              ...
              >
                ...
                <span className="text-base font-medium text-black">Sign in with Apple</span>
              </button>
            </div>
          </div>
        )
  ...
  return (
    <div className="min-h-screen relative flex items-center justify-center py-8">
      ...
      <button
        onClick={handleBack}
        className="absolute top-8 left-8 z-20 w-12 h-12 bg-white/20 backdrop-blur-sm rounded-full flex items-center justify-center text-white hover:bg-white/30 transition-colors"
      >
