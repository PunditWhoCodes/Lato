...
export function ListingTourCard({
					...
					<div className="flex-1 p-4 pt-2 sm:pt-4 flex flex-col">
						...
						<div className="flex justify-between items-start gap-2 sm:block">
							<div className="flex-1">
								...
								{/* Subtitle (Peru) */}
								<p className="text-[#818181] text-[10px] sm:text-[12px] mb-0.5">
									{tour.subtitle}
								</p>

								{/* Location (Ecuador Packages) */}
								<p className="text-[#818181] text-[10px] sm:text-[12px] mb-2 sm:mb-3">
									{tour.location}
								</p>
							</div>
							...
							<div className="flex flex-col items-end gap-1 sm:hidden shrink-0">
								<div className="flex items-center gap-1">
									...
									<span className="text-[10px] text-[#495560] font-medium">
										{tour.rating}
									</span>
								</div>
								<div className="flex items-center gap-1">
					...
					<div className="hidden sm:flex flex-col items-end justify-between p-4">
						<div className="flex flex-col items-end gap-2">
							...
							<div className="flex items-center gap-1">
								...
								<span className="text-[11px] text-[#495560] font-medium mt-0.5">
									{tour.rating}
								</span>
							</div>
	...
	return (
		<article
			data-tour-card
			onClick={onClick}
			className="bg-white rounded-[12px] overflow-hidden cursor-pointer group border border-[#E8E8E8] hover:shadow-md transition-shadow flex flex-col h-full"
		>
			...
			<div className="p-2 pb-0">
				<div className="relative aspect-square rounded-[10px] overflow-hidden">
					...
					{hasDiscount && (
						<div className="absolute bottom-2.5 left-2.5 bg-[#F23813] text-white px-3 py-1 rounded-full text-[10px] font-semibold">
							-{discountPercent}% OFF
						</div>
					)}
				</div>
			...
			<div className="p-3 pt-3 flex flex-col flex-1">
				...
				<div className="flex justify-between items-start gap-2 mb-1.5">
					<div className="flex-1 min-w-0">
						{/* Title */}
						<h3 className="text-[#1C1B1F] font-poppins font-semibold text-[14px] leading-tight mb-1 group-hover:text-[#00A792] transition-colors line-clamp-1">
							{tour.title}
						</h3>
					...
					<div className="flex flex-col items-end gap-1.5 shrink-0">
						...
						<div className="flex items-center gap-1">
							<Star className="w-3 h-3 fill-[#FFA432] text-[#FFA432]" />
							<span className="text-[11px] text-[#1C1B1F] font-medium">
								{tour.rating}
							</span>
						</div>
				...
				{/* Price Row */}
				<div className="flex items-baseline gap-1.5 mb-2.5 mt-2">
					<span className="text-[#818181] text-[11px]">From</span>
					{tour.originalPrice && (
						<span className="text-[#7BBCB0] text-[11px] line-through">
							${tour.originalPrice.toFixed(2)}
						</span>
					)}
					<span className="text-[#F23813] font-semibold text-[14px]">
						${tour.price.toFixed(2)}
					</span>
				</div>
